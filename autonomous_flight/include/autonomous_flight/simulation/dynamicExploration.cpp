...
namespace AutoFlight{
    ...
    ROS_INFO("[AutoFlight]: Auto-starting without waiting for ENTER.");

    // langsung takeoff
    this->takeoff();

    // ROS log takeoff success
    ROS_INFO("[AutoFlight]: Takeoff succeed, auto-continue to planning.");

    // langsung buat rosbag
    ...
    if (temp1==-1 or temp2==-1 or temp3==-1){
        ROS_WARN("[AutoFlight]: Recording fails.");
    }

    // init exploration
    this->initExplore();

    // langsung register callback & start planning
